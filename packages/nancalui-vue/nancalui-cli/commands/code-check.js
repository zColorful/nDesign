...
const entryDir = path.resolve(__dirname, '../../nancalui');
...
const eslintCheckSingle = async (name) => {
  const eslintResult = await shell.exec(`eslint --color "./nancalui/${name}/**/{*.ts,*.tsx}"`);
...
const unitTestSingle = async (name) => {
  const unitTestResult = await shell.exec(`pnpm test --filter vue-nancalui -- \
...
const unitTestAll = async () => {
  ...
  const unitTestResult = await shell.exec('pnpm --filter vue-nancalui test --reporter default');
