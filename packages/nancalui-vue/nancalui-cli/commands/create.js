...
const {
  ...
  DOCS_FILE_NAME,
  VUE_nancalui_FILE,
  VUE_nancalui_IGNORE_DIRS,
  VUE_nancalui_FILE_NAME,
...
const { createVuenancaluiTemplate } = require('../templates/vue-nancalui');
...
async function createComponent(params = {}) {
  ...
  const componentDir = resolve(nancalui_DIR, componentName);
  const srcDir = resolve(componentDir, 'src');
  const testsDir = resolve(nancalui_DIR, componentName, TESTS_DIR_NAME);
...
async function createVuenancalui(params, { ignoreParseError, env }) {
  const fileInfo = resolveDirFilesInfo(nancalui_DIR, VUE_nancalui_IGNORE_DIRS).filter(
  ...
  const template = createVuenancaluiTemplate(exportModules);

  const spinner = ora(`开始创建 ${VUE_nancalui_FILE_NAME} 文件...`).start();

  try {
    await fs.writeFile(VUE_nancalui_FILE, template, { encoding: 'utf-8' });

    spinner.succeed(`${VUE_nancalui_FILE_NAME} 文件创建成功！`);
    logger.info(`文件地址：${VUE_nancalui_FILE}`);
  } catch (e) {
    spinner.fail(e.toString());
    process.exit(1);
  }
...
async function createVitepressSidebar() {
  ...
  const fileInfo = resolveDirFilesInfo(nancalui_DIR, VUE_nancalui_IGNORE_DIRS);
...
exports.create = async (cwd) => {
  ...
  try {
    ...
    case CREATE_SUPPORT_TYPE_MAP.component:
      ...
      break;
    case CREATE_SUPPORT_TYPE_MAP['vue-nancalui']:
      // 创建 nancalui/vue-nancalui.ts
      await createVuenancalui(params, cwd);
