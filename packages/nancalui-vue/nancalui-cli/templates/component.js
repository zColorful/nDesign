const { nancalui_NAMESPACE, CSS_CLASS_PREFIX } = require('../shared/constant');
...
export default defineComponent({
  name: '${bigCamelCase(nancalui_NAMESPACE)}${bigCamelCase(componentName)}',
...
.#{$nancalui-prefix}-${componentName} {
  //
}
`;

// 创建index模板
exports.createIndexTemplate = ({ title, category, hasComponent, hasDirective, hasService, componentName, directiveName, serviceName }) => {
  ...
  const importStr =
    getPartStr(hasComponent, importComponentStr) + getPartStr(hasDirective, importDirectiveStr) + getPartStr(hasService, importServiceStr);
...
// 创建测试模板
exports.createTestsTemplate = ({ componentName, directiveName, serviceName, hasComponent, hasDirective, hasService }) => `\
...
\`\`\`vue
<template>
  <n-${componentName}>{{ data }}</n-${componentName}>
</template>
