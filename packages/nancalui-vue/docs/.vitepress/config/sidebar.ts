      export default {
        '/': [
	  {
	    "text": "快速开始",
	    "link": "/quick-start/"
	  },
	  {
	    "text": "按需引入",
	    "link": "/on-demand/"
	  },
	  {
	    "text": "主题定制",
	    "link": "/theme-guide/"
	  },
	  {
	    "text": "通用",
	    "children": [
	      {
	        "text": "Button 按钮",
	        "link": "/components/button/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Divider 分割线",
	        "link": "/components/divider/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Dragdrop 拖拽",
	        "link": "/components/dragdrop/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Fullscreen 全屏",
	        "link": "/components/fullscreen/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Icon 图标",
	        "link": "/components/icon/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "ModuleName 标题",
	        "link": "/components/module-name/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Overlay 遮罩层",
	        "link": "/components/overlay/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Panel 面板",
	        "link": "/components/panel/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Ripple 水波纹",
	        "link": "/components/ripple/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Scrollbar 滚动条",
	        "link": "/components/scrollbar/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Search 搜索框",
	        "link": "/components/search/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Status 状态",
	        "link": "/components/status/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Sticky 便贴",
	        "link": "/components/sticky/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "VirtualList 虚拟列表",
	        "link": "/components/virtual-list/",
	        "status": "100%",
	        "hide": false
	      }
	    ]
	  },
	  {
	    "text": "导航",
	    "children": [
	      {
	        "text": "Accordion 手风琴",
	        "link": "/components/accordion/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Anchor 锚点",
	        "link": "/components/anchor/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "BackTop 回到顶部",
	        "link": "/components/back-top/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Breadcrumb 面包屑",
	        "link": "/components/breadcrumb/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Dropdown 下拉菜单",
	        "link": "/components/dropdown/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Menu 菜单",
	        "link": "/components/menu/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "NavSprite 导航精灵",
	        "link": "/components/nav-sprite/",
	        "status": "10%",
	        "hide": true
	      },
	      {
	        "text": "Pagination 分页",
	        "link": "/components/pagination/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "StepsGuide 操作指引",
	        "link": "/components/steps-guide/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Tabs 选项卡",
	        "link": "/components/tabs/",
	        "status": "100%",
	        "hide": false
	      }
	    ]
	  },
	  {
	    "text": "反馈",
	    "children": [
	      {
	        "text": "Alert 警告",
	        "link": "/components/alert/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Drawer 抽屉板",
	        "link": "/components/drawer/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Loading 加载提示",
	        "link": "/components/loading/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Message 全局提示",
	        "link": "/components/message/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "MessageBox 二次确认弹框",
	        "link": "/components/message-box/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Modal 弹窗",
	        "link": "/components/modal/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Notification 全局通知",
	        "link": "/components/notification/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Popover 悬浮提示",
	        "link": "/components/popover/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "ReadTip 阅读提示",
	        "link": "/components/read-tip/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Result 结果",
	        "link": "/components/result/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Tooltip 提示",
	        "link": "/components/tooltip/",
	        "status": "100%",
	        "hide": false
	      }
	    ]
	  },
	  {
	    "text": "数据录入",
	    "children": [
	      {
	        "text": "AutoComplete 自动补全",
	        "link": "/components/auto-complete/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Cascader 级联菜单",
	        "link": "/components/cascader/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Checkbox 复选框",
	        "link": "/components/checkbox/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "ColorPicker 颜色选择器",
	        "link": "/components/color-picker/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "DatePicker 日期选择器",
	        "link": "/components/date-picker/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "DatePickerPro 日期选择器",
	        "link": "/components/date-picker-pro/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "EditableSelect 可输入下拉选择框",
	        "link": "/components/editable-select/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Form 表单",
	        "link": "/components/form/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Input 输入框",
	        "link": "/components/input/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "InputIcon输入框",
	        "link": "/components/input-icon/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "InputNumber 数字输入框",
	        "link": "/components/input-number/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Mention 提及",
	        "link": "/components/mention/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "MultiAutoComplete 多项自动补全",
	        "link": "/components/multi-auto-complete/",
	        "status": "10%",
	        "hide": true
	      },
	      {
	        "text": "Radio 单选框",
	        "link": "/components/radio/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Select 下拉框",
	        "link": "/components/select/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Slider 滑块",
	        "link": "/components/slider/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Switch 开关",
	        "link": "/components/switch/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "TagInput 标签输入框",
	        "link": "/components/tag-input/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Textarea 多行文本框",
	        "link": "/components/textarea/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "TimePicker 时间选择器",
	        "link": "/components/time-picker/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "TimeSelect 时间选择器",
	        "link": "/components/time-select/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Transfer 穿梭框",
	        "link": "/components/transfer/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "TreeSelect 树形选择框",
	        "link": "/components/tree-select/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Upload 上传",
	        "link": "/components/upload/",
	        "status": "100%",
	        "hide": false
	      }
	    ]
	  },
	  {
	    "text": "数据展示",
	    "children": [
	      {
	        "text": "Avatar 头像",
	        "link": "/components/avatar/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Badge 徽标",
	        "link": "/components/badge/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Calendar 日历",
	        "link": "/components/calendar/",
	        "status": "1%",
	        "hide": true
	      },
	      {
	        "text": "Card 卡片",
	        "link": "/components/card/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Carousel 走马灯",
	        "link": "/components/carousel/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Collapse 折叠面板",
	        "link": "/components/collapse/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Comment 评论",
	        "link": "/components/comment/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Countdown 倒计时",
	        "link": "/components/countdown/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Descriptions 描述列表",
	        "link": "/components/descriptions/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Empty 空状态",
	        "link": "/components/empty/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Gantt 甘特图",
	        "link": "/components/gantt/",
	        "status": "10%",
	        "hide": true
	      },
	      {
	        "text": "ImagePreview 图片预览",
	        "link": "/components/image-preview/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "List 列表",
	        "link": "/components/list/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Progress 进度条",
	        "link": "/components/progress/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "QuadrantDiagram 象限图",
	        "link": "/components/quadrant-diagram/",
	        "status": "10%",
	        "hide": true
	      },
	      {
	        "text": "Rate 评分",
	        "link": "/components/rate/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Skeleton 骨架屏",
	        "link": "/components/skeleton/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Statistic 统计数值",
	        "link": "/components/statistic/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Steps 步骤条",
	        "link": "/components/steps/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Table 表格",
	        "link": "/components/table/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "TableV 虚拟列表",
	        "link": "/components/table-v/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Tag 标签",
	        "link": "/components/tag/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Timeline 时间轴",
	        "link": "/components/timeline/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Tree 树",
	        "link": "/components/tree/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Watermark 水印",
	        "link": "/components/watermark/",
	        "status": "100%",
	        "hide": false
	      }
	    ]
	  },
	  {
	    "text": "布局",
	    "children": [
	      {
	        "text": "Grid 栅格",
	        "link": "/components/grid/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Layout 布局",
	        "link": "/components/layout/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Space 间距",
	        "link": "/components/space/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "Splitter 分割器",
	        "link": "/components/splitter/",
	        "status": "100%",
	        "hide": false
	      }
	    ]
	  },
	  {
	    "text": "复合组件",
	    "children": [
	      {
	        "text": "CardList 卡片列表",
	        "link": "/components/card-list/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "CustomizeSteps 自定义步骤条",
	        "link": "/components/customize-steps/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "DynamicWiden 拖拽左右布局",
	        "link": "/components/dynamic-widen/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "HandleWidth 拖拽宽高组件",
	        "link": "/components/handle-width/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "ImportErrModal 错误提示框",
	        "link": "/components/import-err-modal/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "LeftTree 左侧树",
	        "link": "/components/left-tree/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "MyTable 复合列表",
	        "link": "/components/my-table/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "PermissionButton 权限按钮",
	        "link": "/components/permission-button/",
	        "status": "100%",
	        "hide": false
	      },
	      {
	        "text": "PublicTable 复合列表",
	        "link": "/components/public-table/",
	        "status": "100%",
	        "hide": false
	      }
	    ]
	  }
	]
      }
      