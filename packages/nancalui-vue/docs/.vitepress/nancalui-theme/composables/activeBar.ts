...
export function useActiveSidebarLinks(container: Ref<HTMLElement>, marker: Ref<HTMLElement>) {
  ...
  function activateLink(hash: string) {
    ...
    const activeLink = (prevActiveLink =
      hash == null ? null : (container.value.querySelector(`.nancalui-item a[href="${decodeURIComponent(hash)}"]`) as HTMLAnchorElement));
    if (activeLink) {
...
function getSidebarLinks() {
  return Array.from(document.querySelectorAll('.nancalui-content-nav .nancalui-link')) as HTMLAnchorElement[];
}
