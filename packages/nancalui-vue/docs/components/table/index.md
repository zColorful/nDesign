...
:::demo 简单表格，`n-table`组件上的`data`属性传入要展示的数据，`n-column`组件上通过`field`传入对应列内容的字段名，`header`传入对应列的标题。

```vue
<template>
  <n-button @click="setRowHighLight">设置高亮</n-button>
  <n-table :data="baseTableData" style="width: 100%" ref="tableRef" rowKey="firstName">
    <n-column field="firstName" header="First Name"></n-column>
    <n-column field="lastName" header="Last Name"></n-column>
    <n-column field="gender" header="Gender"></n-column>
    <n-column field="date" header="Date of birth"></n-column>
  </n-table>
</template>
...
export default defineComponent({
  setup() {
    ...
    function setRowHighLight() {
      tableRef.value.store.setRowHighLight('green');
    }
    return { baseTableData, setRowHighLight, tableRef };
  },
});
</script>
```
...
<template>
  <div class="table-btn-groups mb-2">
    <div class="table-btn mb-1">
      自动表格布局：
      <n-switch v-model="tableLayout" />
    </div>
    <div class="table-btn mb-1">
      斑马纹：
      <n-switch v-model="striped" />
    </div>
    <div class="table-btn mb-1">
      表头背景色：
      <n-switch v-model="headerBg" />
    </div>
    <div class="table-btn mb-1">
      表格大小：
    ...
    <div class="table-btn mb-1">
      表格边框：
      <n-radio-group direction="row" v-model="borderType">
        <n-radio v-for="item in borderTypeList" :key="item.label" :value="item.value">
          {{ item.label }}
        </n-radio>
      </n-radio-group>
    </div>
  ...
  >
    <n-column field="firstName" header="First Name"></n-column>
    <n-column field="lastName" header="Last Name" show-overflow-tooltip></n-column>
    <n-column field="gender" header="Gender"></n-column>
    <n-column field="date" header="Date of birth"></n-column>
  </n-table>
</template>
...
export default defineComponent({
  setup() {
    ...
    setTimeout(() => {
      stripedTableData.value[0].lastName = '12313y981239812398123';
    }, 2000);
...
:::demo 通过添加一个`n-column`并且设置`type`属性为`checkable`即可实现表格的多选功能。`getCheckedRows`方法可以获取已选择的列表。`toggleRowSelection`方法可以切换某一行的选中状态。通过`cell-click`事件监听单元格点击，事件回调参数包含行索引、列索引、行数据、列数据。在列上配置`resizeable`属性，可实现该列拖动改变宽度，`min-width`和`max-width`设置可拖动范围，事件`resize-start`、`resizing`、`resize-end`分别在拖动开始时、进行中、结束后触发。

```vue
<template>
  <div>
    <div class="mb-2">
      <n-button @click="handleClick" class="mr-1">Get CheckedRows</n-button>
      <n-button @click="insertRow" class="mr-1">Insert Row</n-button>
      <n-button @click="deleteRow" class="mr-1">Delete Row</n-button>
      <n-button @click="toggleRow" class="mr-1">Toggle Row</n-button>
    </div>
    <n-table
      ref="tableRef"
      :data="data"
    ...
    >
      <n-column type="checkable" width="40" :checkable="checkable" reserve-check></n-column>
      <n-column field="firstName" header="First Name" width="200"></n-column>
      <n-column
        field="lastName"
        header="Last Name"
        width="200"
...
:::demo 通过添加一个`n-column`并且设置`type`参数为`index`即可给表格添加索引。索引列的表头默认展示`#`，可通过`header`参数传入指定内容。

```vue
<template>
  <div>
    <n-table :data="data">
      <n-column type="index" width="40"></n-column>
      <n-column field="firstName" header="First Name"></n-column>
      <n-column field="lastName" header="Last Name"></n-column>
      <n-column field="gender" header="Gender"></n-column>
      <n-column field="date" header="Date of birth"></n-column>
...
:::demo 通过`n-column`子组件提供的`default`插槽可以实现自定义列，插槽提供`row`和`rowIndex`两个参数，分别代表行数据和行索引值。

```vue
<template>
  <n-table :data="dataSource">
    <n-column type="index" width="80">
    ...
    <n-column field="gender" header="Gender"></n-column>
    <n-column field="date" header="Date of birth"></n-column>
    <n-column header="Operation" align="right">
      <template #default="scope">
        <n-button @click="handleClick(scope.row)">编辑</n-button>
      </template>
    </n-column>
  </n-table>
</template>
...
:::demo 通过`n-column`子组件提供的`cell`、`cellEdit`插槽以及属性`type`设置为`editable`实现编辑单元格。此功能需配合`row-key`属性使用。

```vue
<template>
  <n-table ref="tableRef" :data="dataSource" row-key="id" @cellClick="cellClick">
    <n-column type="index" width="80">
      <template #default="scope">
        {{ `No.${scope.rowIndex + 1}` }}
      </template>
    </n-column>
    <n-column field="firstName" header="First Name" type="editable">
      <template #cell="scope">
        {{ scope.row.firstName }}
      </template>
    ...
    <n-column field="lastName" header="Last Name" type="editable">
      <template #cell="scope">
        {{ scope.row.lastName }}
      </template>
      <template #cellEdit="scope">
    ...
    <n-column field="gender" header="Gender" type="editable">
      <template #cell="scope">
        {{ scope.row.gender }}
      </template>
      <template #cellEdit="scope">
...
:::demo 通过`n-column`子组件提供的`header`插槽可以实现自定义表头。

```vue
<template>
  <n-table :data="dataSource">
    <n-column field="firstName">
      <template #header>
        <div>
          <span style="margin-right:4px;font-size:var(--nancalui-font-size,12px)">First Name</span>
          <n-popover content="some tips text" trigger="hover" :position="['top']">
            <n-icon name="info-o"></n-icon>
          </n-popover>
        </div>
      </template>
    </n-column>
...
<template>
  <div>
    <n-button @click="handleClick">更新数据</n-button>
    <n-table :data="emptyData" :show-loading="showLoading">
      <n-column field="firstName" header="First Name"></n-column>
      <n-column field="lastName" header="Last Name"></n-column>
      <n-column field="gender" header="Gender"></n-column>
      <n-column field="date" header="Date of birth"></n-column>
...
```vue
<template>
  <n-table :data="dataSource" table-height="200px" fix-header style="width:100%">
    <n-column field="firstName" header="First Name" :width="150"></n-column>
    <n-column field="lastName" header="Last Name" :width="150"></n-column>
    <n-column field="gender" header="Gender" :width="150"></n-column>
    <n-column field="date" header="Date of birth" :width="150"></n-column>
  </n-table>
</template>
...
```vue
<template>
  <n-table :data="tableDataFixedColumn" table-layout="auto">
    <n-column field="idNo" header="ID Card Number" fixed-left="0px"></n-column>
    <n-column field="firstName" header="First Name" min-width="200px"></n-column>
    <n-column field="lastName" header="Last Name" min-width="200px"></n-column>
    <n-column field="gender" header="Gender" min-width="200px"></n-column>
    <n-column field="date" header="Date of birth" min-width="200px"></n-column>
    <n-column field="address" header="Address" min-width="200px"></n-column>
    <n-column field="occupation" header="Occupation" min-width="200px"></n-column>
    <n-column field="occupation" header="Occupation" min-width="200px"></n-column>
    <n-column field="occupation" header="Occupation" fixed-right="100px"></n-column>
    <n-column field="occupation" header="Occupation" fixed-right="10px"></n-column>
  </n-table>
</template>
...
```vue
<template>
  <n-table :data="dataSource" :span-method="spanMethod" border-type="bordered">
    <n-column field="firstName" header="First Name"></n-column>
    <n-column field="lastName" header="Last Name"></n-column>
    <n-column field="gender" header="Gender"></n-column>
    <n-column field="date" header="Date of birth"></n-column>
...
:::demo `n-column`嵌套`n-column`即可实现表头分组。

```vue
<template>
  <n-table :data="dataSource">
    <n-column field="name" header="Name">
      <n-column field="firstName" header="First Name"></n-column>
      <n-column field="lastName" header="Last Name"></n-column>
    </n-column>
    <n-column field="gender" header="Gender"></n-column>
...
```vue
<template>
  <n-table :data="dataSource" @sort-change="handleSortChange">
    <n-column field="firstName" header="First Name"></n-column>
    <n-column field="lastName" header="Last Name" sortable :sort-method="sortNameMethod"></n-column>
    <n-column field="gender" header="Gender"></n-column>
    <n-column field="date" header="Date of birth" sortable sort-direction="ASC" :sort-method="sortDateMethod"></n-column>
  </n-table>
</template>
...
<template>
  <n-table :data="dataSource">
    <n-column field="firstName" header="First Name"></n-column>
    <n-column field="lastName" header="Last Name" filterable :filter-list="filterList" @filter-change="handleFilterChange"></n-column>
    <n-column
      field="gender"
      header="Gender"
      filterable
...
<template>
  <div>
    <n-button @click="toggleRowExpansion" class="mr-m mb-m">toggleRowExpansion</n-button>
    <n-table ref="tableRef" :data="dataSource" row-key="id" :expand-row-keys="['1', '3']" @expand-change="expandChange">
      <n-column type="expand">
...
<template>
  ...
  >
    ...
    <n-column field="name" header="First Name" show-overflow-tooltip></n-column>
    <n-column field="lastName" header="Last Name">
      <template #default="scope">
        <span @click="scope.row.lastName += 1">{{ scope.row.lastName }}</span>
      </template>
    </n-column>
    <n-column field="gender" header="Gender"></n-column>
    <n-column field="date" header="Date of birth"></n-column>
...
export default defineComponent({
  setup() {
    const baseTreeTableData = ref([
                ...
                children: [
                  {
                    ...
                    description: '测试删除',
                    children: null,
                    fullId: '436/438//440/438/440/444',
                    fullName: '测试组-01/测试子类/测试子类修改父级/测试删除',
                    name: '测试删除',
                  },
                ],
                fullId: '436/438//440',
                fullName: '测试组-01/测试子类/测试子类修改父级',
                name: '测试子类修改父级',
              },
            ],
            fullId: '436/438',
            fullName: '测试组-01/测试子类',
            name: '测试子类',
          },
        ],
        fullId: '436',
      ...
      { id: '458', parentId: '0', label: '组织2', description: '2', children: null, fullId: '458', fullName: '组织2', name: '组织2' },
      {
        id: '464',
        parentId: '0',
        label: '组织3',
    ...
    const expandedChange = (item) => {
      if (expandedArray.value.includes(item.firstName)) {
        expandedArray.value = expandedArray.value.filter((expandedItem) => expandedItem !== item.firstName);
      } else {
        expandedArray.value.push(item.firstName);
      }
      console.log([...expandedArray.value]);
    };

    return { baseTreeTableData, treeCheckChange, expandedArray, expandedChange };
  },
});
</script>
```
...
```vue
<template>
  <n-table :data="dataSource" table-height="200px" :show-loading="showLoading" :lazy="true" @load-more="loadMore">
    <n-column field="firstName" header="First Name"></n-column>
    <n-column field="lastName" header="Last Name"></n-column>
    <n-column field="gender" header="Gender"></n-column>
    <n-column field="date" header="Date of birth"></n-column>
...
:::demo 设置 virtualized 开启虚拟列表功能。目前虚拟滚动表格受限比较多，开启虚拟滚动后不能使用展开行、树形数据、固定列等功能

```vue
<template>
  <n-table :data="baseTableData" table-height="300px" virtualized style="width: 100%">
    <n-column field="key" header="First Name"></n-column>
    <n-column field="name" header="Last Name"></n-column>
    <n-column field="address" header="Address"></n-column>
    <n-column field="email" header="Email"></n-column>
  </n-table>
</template>
...
| size                  | [TableSize](#tablesize)                           | 'md'      | 可选，表格大小，分别对应行高 40px,50px,60px                                                                                                    | [表格样式](#表格样式)                                          |
