...
export default defineComponent({
  ...
  emits: ['addMilestone'],
  setup(props, ctx) {
    const { startDate, endDate, milestoneList, scrollElement, unit } = toRefs(props);
    ...
    const getViewScaleData = () => {
      if (scrollElement.value) {
        ...
        viewSCaleData.value = scaleData.value.filter((i: GanttScaleDateInfo) => {
    ...
    watch(
      ...
      () => {
        getViewScaleData();
        (props.scrollElement as HTMLDivElement).addEventListener('scroll', () => {
          getViewScaleData();
        });
      }
    );
  ...
  render() {
    const { unit, viewSCaleData, scaleWidth, addMilestone, highlight, highlightStartText, highlightEndText, ganttBarContainerElement } =
      this;
    return (
      <div class="nancalui-gantt-scale-wrapper">
        {viewSCaleData.map((data, index) => (
          <div
            class={`nancalui-gantt-scale ${unit} ${data.today} ${data.milestone}`}
            style={{
            ...
            <div class="nancalui-scale-unit">
              ...
              {(!highlight || !data.highlightStart) && unit === 'day' && (
                <div class={`border-left ${data.today ? 'today' : ''}`}>{data.today ? i18nText.zh.today : data.dayOfMonthLabel}</div>
              )}
              ...
              {(!highlight || !data.highlightStart) && unit === 'month' && (
                <div class={`${data.monthStart || index === 0 ? 'border-left' : ''}`}>
                  {index === 0 || data.monthStart ? i18nText.zh.monthDisplay(data.monthLabel) : ''}
                </div>
              )}
            </div>
            <div class={`milestone-new ${unit}`} title="milestone" onClick={() => addMilestone(data)}>
              <n-icon name="add" />
            </div>
          </div>
        ))}
