...
export function initSelect(defaultSelectKeys: any, keys: string, isMultiple: boolean, disabled: Ref<boolean>): boolean {
  const isSelect = ref(false);
  if (!isMultiple) {
    if (defaultSelectKeys?.value[0] === keys && !disabled.value) {
      isSelect.value = true;
    } else {
      isSelect.value = false;
    }
  } else {
    if (defaultSelectKeys?.value?.includes(keys)) {
      isSelect.value = true;
    } else {
      isSelect.value = false;
    }
