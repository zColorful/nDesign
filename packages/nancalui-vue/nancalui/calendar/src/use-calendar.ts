...
const debugWarn = (name: any, msg: any) => {
  console.error(name + ':' + msg);
};

import type { ComputedRef, SetupContext } from 'vue';
import type { Dayjs } from 'dayjs';
import type { CalendarDateType, CalendarEmits, CalendarProps } from './calendar-types';
...
export const useCalendar = (props: CalendarProps, emit: SetupContext<CalendarEmits>['emit'], componentName: string) => {
  ...
  const validatedRange = computed(() => {
    ...
    const rangeArrDayjs = props.range.map((_: any) => dayjs(_).locale('zh-cn'));
    const [startDayjs, endDayjs]: Dayjs[] = rangeArrDayjs;
