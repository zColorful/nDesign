...
import { ComponentPublicInstance, computed, defineComponent, nextTick, onMounted, PropType, ref, toRefs, watch } from 'vue';
...
import TabsNavInk from './tabs-nav-ink';
import type { TabData, TabsType } from './interface';
import ResizeObserver from '../../shared/components/resize-observer';
...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const mergedEditable = computed(() => ['line', 'card', 'card-gutter'].includes(props.type));
    ...
    const getTabEndOffsets = () => {
      return tabs.value.map((item) => {
        const ele = tabsRef.value[item.id];
        if (direction.value === 'vertical') {
    ...
    const getValidOffset = (off: number) => {
      if (!wrapperRef.value || !listRef.value || off < 0) {
        return 0;
      }
      if (off > maxOffset.value) {
        return maxOffset.value;
      }
      return off;
    };

    const handleClick = (item: TabData, ev: Event) => {
      emit('click', item, ev);
    };
    ...
    const listCls = computed(() => [
      `${prefixCls}-tab-list`,
      {
        [`${prefixCls}-tab-list-no-padding`]:
          !props.headerPadding && ['line', 'text'].includes(props.type) && props.direction === 'horizontal',
      },
    ]);
    ...
    return () => (
      <div class={cls.value}>
        {isScroll.value && (
          <TabsButton type="previous" direction={props.direction} disabled={offset.value <= 0} onClick={handleButtonClick} />
        )}
              ...
              <div ref={listRef} class={listCls.value} style={listStyle.value}>
                {tabs.value.map((tab, index) => (
                  <TabsTab
                    ...
                    ref={(component: any) => {
                      if (component?.$el) {
                        tabsRef.value[tab.id] = component.$el;
                      }
                    }}
                    active={tab.id === activeKey.value}
                    tab={tab}
                    editable={props.editable && (tab.closable ?? true)}
                    onClick={handleClick}
