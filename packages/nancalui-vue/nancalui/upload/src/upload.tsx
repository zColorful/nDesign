...
export default defineComponent({
  ...
  setup(props: UploadProps, ctx) {
    ...
    const t = createI18nTranslate('NUpload', app);
                      ...
                      {fileUploader.status === UploadStatus.uploading && (
                        <div class={['icon', ns.e('progress')]}>
                          <n-progress
                            ...
                            barbgcolor="#50D4AB"
                            strokeWidth={8}
                            showContent={false}></n-progress>
                        </div>
                      )}
                      {fileUploader.status === UploadStatus.failed && <n-icon name="running" onClick={fileUpload} />}
                      {fileUploader.status === UploadStatus.uploaded && <n-icon name="right" color="#50d4ab" />}
                    </li>
                  ))}
                </ul>
              )}
              {/* <span class={inputGroupAddOnNs.b()}>
                <n-icon name="more-operate" color="#252b3a" />
              </span> */}
            </div>
          )}
        </div>
