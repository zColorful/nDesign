...
describe('upload', () => {
  ...
  it('should work with `before-upload auto-upload withoutBtn http-request` prop', async () => {
    ...
    await nextTick();
    const input = document.getElementById('n-upload-temp');
    const fileList = [
  ...
  it('should work with `auto-upload  on-change` prop and `submit` func', async () => {
    ...
    const TestComponent = {
      ...
      template: `
        <div>
          <n-upload ref="demoUpload" v-model="uploadedFiles" :upload-options="uploadOptions" :auto-upload="false" :on-change="onChange" />
  ...
  it('should work with `on-progress on-preview` hooks', async () => {
    ...
    const TestComponent = {
      ...
      template: `
        <div>
          <n-upload v-model="uploadedFiles" :upload-options="uploadOptions" :on-progress="onProgress" :on-preview="onPreview" />
        </div>
        `,
      setup() {
        const uploadedFiles = ref([]);
