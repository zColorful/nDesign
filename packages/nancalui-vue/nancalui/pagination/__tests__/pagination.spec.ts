...
describe('pagination: ', () => {
  it('test pageSize', async () => {
    ...
    await nextTick();
    expect(wrapper.find('.nancalui-icon__container span').text()).toEqual('20');
    ...
    // // 改变每页条数
    await wrapper.find('.nancalui-icon__container span').trigger('click');
    const selectItems = document.querySelectorAll('.nancalui-pagination__size-list li');
    await selectItems[1].dispatchEvent(new Event('click'));

    expect(wrapper.find('.nancalui-icon__container span').text()).toEqual('10');
