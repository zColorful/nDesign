  ...
  setup(props: PaginationProps, { emit, slots }) {
    ...
    return () =>
      ...
      autoHide.value && Math.min(...pageSizeOptions.value) > total.value ? null : (
        <div class={ns.b()}>
          {layout.value.split(',').map((item) => {
            // 总页数显示
            if (item === 'total' && (!lite.value || (lite.value && showPageSelector.value)) && canViewTotal.value) {
              return <div class={ns.e('total-size')}>{totalItemText?.value || t('totalItemText')(total.value)}</div>;
            }
            ...
            // 跳转页码
            if (item === 'jumper' && canJumpPage.value && !lite.value) {
              return (
                <jump-page
                  {...{
            ...
            if (item === 'sizes') {
              // 切换每页数据大小的下拉框
              if (canChangePageSize.value && !lite.value) {
                return <PageSize />;
              }
              // 极简模式下的选择页码下拉框
              if (lite.value && showPageSelector.value) {
                return (
                  <Select
