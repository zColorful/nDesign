...
export default defineComponent({
  setup() {
    ...
    return () => (
      <>
        <Dropdown position={['bottom', 'top']} overlay-class="page-dropDown" class={ns.e('size-list')} onToggle={onDropdownToggle}>
          {{
            default: () => (
              ...
              <ul>
                {pageSizeOptions.value.map((item, index) => (
                  <li
                    ...
                    onClick={withModifiers(pageSizeChange.bind(null, { value: item }), ['prevent'])}
                    key={index}>
                    {item}
                  </li>
                ))}
