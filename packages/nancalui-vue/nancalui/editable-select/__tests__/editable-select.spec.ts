...
describe('editable-select test', () => {
  ...
  test('should render correctly', async () => {
    ...
    expect(wrapper.find('.nancalui-dropdown-item').exists()).toBeFalsy();

    await input.trigger('click');
    const flexibleOverlay = wrapper.getComponent({ name: 'DFlexibleOverlay' });

    expect(flexibleOverlay.find(`.nancalui-dropdown-item`).exists()).toBeTruthy();
    expect(wrapper.classes()).toContain('nancalui-editable-select--open');

    const options = flexibleOverlay.findAll(`.nancalui-dropdown-item`);
  ...
  test('select on click ', async () => {
    ...
    const options = flexibleOverlay.find(`.nancalui-dropdown-item`);
  ...
  test('filter option', async () => {
    ...
    await input.trigger('click');
    const flexibleOverlay = wrapper.getComponent({ name: 'DFlexibleOverlay' });
    expect(flexibleOverlay.findAll(`.nancalui-dropdown-item`).length).toBe(1);
  });
  ...
  test('custom filter options', async () => {
    ...
    expect(flexibleOverlay.find(`.nancalui-dropdown-item`).text()).toBe('label0');
  });
  ...
  test('load more ', async () => {
    ...
    await input.trigger('click');
    const flexibleOverlay = wrapper.getComponent({ name: 'DFlexibleOverlay' });
    const ul = flexibleOverlay.find(`.nancalui-editable-select__list--unstyled`);
