...
import { componentToSlot, enforceUnit, tryCall, isFunction } from '../utils';
...
Pick<TableV2Props, 'cellProps' | 'expandColumnKey' | 'indentSize' | 'iconSize' | 'rowKey'> &
...
const CellRenderer: FunctionalComponent<CellRendererProps> = (
  ...
  const CellComponent = columnCellRenderer || slots.default || ((props: CellRendererParams<any>) => <TableCell {...props} />);

  const cellData = isFunction(dataGetter) ? dataGetter({ columns, column, columnIndex, rowData, rowIndex }) : get(rowData, dataKey ?? '');
  ...
  if (expandable) {
    if (isObject(expandIconProps)) {
      ...
      IconOrPlaceholder = <div style={[iconStyle, `width: ${iconSize}px; height: ${iconSize}px;`].join(' ')} />;
    }
  }
