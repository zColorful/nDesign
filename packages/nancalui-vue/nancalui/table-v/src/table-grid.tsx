...
import DynamicSizeGrid from './components/grdi';
import FixedSizeGrid from './components/fixed-size-grid';
...
const useTableGrid = (props: TableV2GridProps) => {
  ...
  const itemKey: any = ({ data, rowIndex }: any) => data[rowIndex][props.rowKey];

  function onItemRendered({ rowCacheStart, rowCacheEnd, rowVisibleStart, rowVisibleEnd }: any) {
  ...
  function scrollTo(options: any): void;
  function scrollTo(leftOrOptions: any, top?: number) {
  ...
  function scrollToRow(row: number, strategy: any) {
...
UnwrapRef<{
  ...
  /**
     ...
     * @param { number } arg2
     */
  scrollTo(leftOrOptions: number | any, top?: number): void;
