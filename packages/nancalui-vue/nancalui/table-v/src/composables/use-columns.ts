...
import type { AnyColumns, ColumnV, KeyType } from '../types';
...
function useColumns(props: TableV2Props, columns: Ref<AnyColumns>, fixed: Ref<boolean>) {
  ...
  const columnsStyles = computed(() => {
    ...
    return _columns.reduce<Record<ColumnV<any>['key'], CSSProperties>>((style, column) => {
  ...
  const getColumnStyle = (key: any) => {
    return unref(columnsStyles)[key];
  };

  const updateColumnWidth = (column: ColumnV<any>, width: number) => {
    column.width = width;
  };
