...
const COMPONENT_NAME = 'NTableV2Header';
const TableV2Header = defineComponent({
  name: COMPONENT_NAME,
  props: tableV2HeaderProps,
  setup(props, { slots, expose }) {
    const ns = useNamespace('table-v-header');
    ...
    const renderFixedRows = () => {
      ...
      return fixedHeaderData?.map((fixedRowData: any, fixedRowIndex: number) => {
    ...
    const renderDynamicRows = () => {
      ...
      return unref(headerHeights).map((rowHeight: any, rowIndex: any) => {
        const style: CSSProperties = enforceUnit({
          width: '100%',
          height: rowHeight,
        });
