...
import { Ref } from 'vue';
import { CascaderOptionInfo, CascaderFieldNames, CascaderOption } from './interface';
...
export const getOptionInfos = (
  ...
  const travelOptions = (opts: CascaderOption[], parent?: CascaderOptionInfo, level?: number) => {
    ...
    return opts.map((item, index) => {
      ...
      const data: CascaderOptionInfo = {
        ...
        index,
        key: '',
        valueKey: String(isObject(value) ? (value as any)[valueKey.value] : value),
        parent,
        path: [],
        pathValue: [],
      };
      ...
      if (data.children && !data.disabled) {
        data.totalLeafOptions = data.children.reduce((pre, d) => {
          if (isNumber(d.totalLeafOptions)) {
            return pre + d.totalLeafOptions;
          }

          if (d.disabled || d.selectionDisabled) {
            return pre;
          }

          return pre + (d.isLeaf ? 1 : 0);
        }, 0);
