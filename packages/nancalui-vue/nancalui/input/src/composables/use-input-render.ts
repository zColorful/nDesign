...
export function useInputRender(props: InputProps, ctx: SetupContext): UseInputRender {
  ...
  const { error, size, disabled, noborder, line } = toRefs(props);
  const slots = ctx.slots;
  const inputDisabled = computed(() => disabled.value || formContext?.disabled);
  const inputDisabledBorder = computed(() => noborder.value || formContext?.noborder);
  const inputLine = computed(() => line.value || '');
  ...
  const wrapClasses = computed(() => ({
    ...
    [ns.m('noborder')]: inputDisabledBorder.value,
    [ns.m('line')]: inputLine.value,
  ...
  return { inputDisabled, inputDisabledBorder, inputSize, isFocus, wrapClasses, inputClasses, customStyle, otherAttrs };
}
