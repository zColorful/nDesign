...
import NTransfer from '../src/transfer';
...
describe('n-transfer', () => {
  it('n-transfer basic work', async () => {
    ...
    await nextTick();
    expect(wrapper.findAll(`${sourceClass} ${bodyClass} .nancalui-checkbox`).length).toBe(7);
    expect(wrapper.findAll(`${targetClass} ${bodyClass} .nancalui-checkbox`).length).toBe(1);
  ...
  it('n-transfer search work', async () => {
    const sourceOption = ref(SOURCE_DATA);
    ...
    expect(wrapper.find(`${sourceClass} .nancalui-search`).exists()).toBe(true);
    ...
    expect(wrapper.find(`${sourceClass} ${bodyClass} .nancalui-checkbox`).text()).toBe('福建');
    ...
    // 目标搜索功能
    expect(wrapper.find(`${targetClass} .nancalui-search`).exists()).toBe(true);
    ...
    expect(wrapper.find(`${targetClass} ${bodyClass} .nancalui-checkbox`).text()).toBe('武汉');
  ...
  it('n-transfer render-content work', async () => {
    const sourceOption = ref(SOURCE_DATA);
    const modelValues = ref(['武汉']);
    const titles = ref(['sourceHeader', 'targetHeader']);
    const filter = ref(true);
    const renderContent = (h, option) => {
      return h('span', { style: { color: '#447DFD' } }, [option.value, option.name]);
    };
    const wrapper = mount({
      components: {
        NTransfer,
    ...
    const firstCheckbox = wrapper.find(`${sourceClass} ${bodyClass} .nancalui-checkbox`);
    const firstCheckboxTextLabel = firstCheckbox.find('.nancalui-checkbox__label-text span'); // the text element
    expect(firstCheckboxTextLabel.text()).toBe('北京北京');

    // check the checkbox props —— renderContent: h('span', { style: { color: '#447DFD' } }, [option.value, option.name])
