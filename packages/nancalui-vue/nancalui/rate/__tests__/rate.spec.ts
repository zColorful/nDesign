...
describe('rate', () => {
  ...
  describe('rate change', () => {
    it('Rate can be changed', async () => {
      ...
      expect(fourthStarEle.find(ns.e('color-active')).attributes('style')).toBe('width: 100%;');

      await container.trigger('mouseleave');
      expect(fourthStarEle.find(ns.e('color-active')).attributes('style')).toBe('width: 0px;');
  ...
  describe('read only', () => {
    const TestComponent = {
      ...
      template: `
        <div>
          <n-rate v-model="value" icon="star-o" :read="true"/>
          <div class="count">当前有{{ value }}颗星</div>
        </div>
      `,
      setup() {
