  ...
  setup(props: colorPickerAlphaSliderProps, ctx) {
    ...
    const onMoveBar = (event: MouseEvent) => {
      ...
      if (barElement.value && cursorElement.value) {
        ...
        ctx.emit('change', fromHSVA({ ...props.modelValue.hsva, a: alpha / 100 }));
      }
    };
    ...
    const alphaClass = computed(() => {
      return ['nancalui-color-picker-alpha-slider', 'transparent'];
    });
    return () => {
      return (
        <div class={alphaClass.value}>
          <div ref={barElement} class="nancalui-color-picker-alpha-slider__bar" style={getBackgroundStyle.value} onClick={onClickSider}>
            <div class={['nancalui-color-picker-alpha-slider__bar-pointer']} ref={cursorElement} style={getCursorStyle.value}>
              <div class="nancalui-color-picker-alpha-slider__bar-handle"></div>
            </div>
          </div>
        </div>
      );
