...
export default defineComponent({
  ...
  setup(props: ColorPickerEditProps, { emit }) {
    ...
    // 模式值
    const modelValue = computed(() => `${props.mode ?? DEFAUTL_MODE}${isShowAlpha.showAlpha ? 'a' : ''}`);
    // 颜色值
    const colorValue = ref<Partial<ColorPickerColor> | undefined>(props.color);
    // 模式值类型
    const modelValueType = computed(() => ((props.mode ?? DEFAUTL_MODE) === 'hex' ? 'string' : 'number'));
    ...
    function renderStringValue() {
      ...
      return (
        <div class="nancalui-color-picker-edit-input string-input flex">
          <div class="nancalui-color-picker-edit-input-wrapper">
            <input value={value} onChange={updateValue} class="nancalui-color-picker-edit-input-value" />
          </div>
        </div>
      );
    }
    ...
    function renderNumberValue() {
      ...
      const key = modelValue.value;
      // 对指定数值进行缩放处理
      const value = colorValue.value?.[key.replace(/a?$/, 'a') as Extract<keyof ColorPickerColor, 'hsla' | 'hsva' | 'rgba'>];
      ...
      return (
        <div class="nancalui-color-picker-edit-input number-input flex">
          {key.split('').map((k: string) => (
            <div class="nancalui-color-picker-edit-input-wrapper">
              <input
                onKeydown={onKeyChangeValue()}
                class="nancalui-color-picker-edit-input-value"
    ...
    return () => (
      <div class="nancalui-color-picker-edit flex">
        <div class="nancalui-color-picker-edit-name" onClick={onChangeModel}>
