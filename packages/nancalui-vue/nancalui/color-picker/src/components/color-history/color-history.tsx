...
export default defineComponent({
  ...
  setup(props: ColorPickerHistoryProps) {
    ...
    return () => (
      <div class="nancalui-color-picker-history flex flex-wrap">
        {history.value.map((hexa: string) => (
          <div
            onClick={() => onChangeColor(hexa)}
            class={['nancalui-color-picker-history_color-box', hexa.endsWith('00') ? 'transparent' : '']}
            style={{ backgroundColor: hexa }}></div>
        ))}
      </div>
    );
  },
