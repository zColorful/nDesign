...
export function useFormLabel(): UseFormLabel {
  ...
  const arrangement = computed(() => ({ [`${labelData.value.layout}`]: true }));
  const labelWidth = computed(() => ({ labelWidth: `${labelData.value.labelWidth}` }));

  return { labelClasses, labelInnerClasses, arrangement, labelWidth };
}
