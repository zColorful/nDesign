...
export function useFormItemValidate(props: FormItemProps, _rules: ComputedRef<FormRuleItem[]>): UseFormItemValidate {
  ...
  const computedField = computed(() => {
    return typeof props.field === 'string' || Array.isArray(props.field) ? props.field : '';
  });
  const fieldValue = computed(() => {
    ...
    let objValue: any = formData;
    if (Array.isArray(computedField.value)) {
      computedField.value.forEach((path) => {
        objValue = getFieldValue(objValue, path).value;
      });
      return objValue;
    } else {
  ...
  const execValidate = async (rules: RuleItem[]) => {
    ...
    if (Array.isArray(ruleName)) {
      ...
      return validator
        .validate({ [ruleName.join('.')]: fieldValue.value }, { firstFields: true })
        .then(() => {
          onValidateSuccess();
          return true;
        })
