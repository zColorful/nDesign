...
describe('result test', () => {
  it('result init render', async () => {
    ...
    expect(wrapper.classes()).toContain('nancalui-result');
  });
  ...
  it('icon', async () => {
    ...
    expect(firstChild.classList).toContain('nancalui-result__icon-info');

    await wrapper.setProps({ icon: 'success' });
    firstChild = wrapper.element.firstElementChild;
    expect(firstChild.classList).toContain('nancalui-result__icon-success');

    await wrapper.setProps({ icon: 'danger' });
    firstChild = wrapper.element.firstElementChild;
    expect(firstChild.classList).toContain('nancalui-result__icon-danger');

    await wrapper.setProps({ icon: 'warning' });
    firstChild = wrapper.element.firstElementChild;
    expect(firstChild.classList).toContain('nancalui-result__icon-warning');
  });
