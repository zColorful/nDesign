 ...
 * @returns
 */
export const formatValue = (value: string|Date, state: TState, props: DatePickerProps): string => {
  const { format = 'y/MM/dd', range, rangeSpliter = '-', modelValue } = props || {};
  ...
  if (range) {
    ...
    return formatRange(format, state.start, state.end, rangeSpliter);
  } else {
    if (!value) {
      return '';
    }
 ...
 * @returns
 */
export const handlePositionFactory =
  (
    state: {
      x?: string;
      y?: string;
