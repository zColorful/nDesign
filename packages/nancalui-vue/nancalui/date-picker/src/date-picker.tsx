...
// import datePickIcon from './assets/icon_calendar_n.svg';
import { TState, datePickerProps, DatePickerProps } from './date-picker-types';

import { handleCalendarSwitchState, formatValue, formatPlaceholder } from './helper';
...
export default defineComponent({
  ...
  setup(props: DatePickerProps, ctx: SetupContext) {
    ...
    state.value = formatValue(props.modelValue, state, props);
    state.placeholder = formatPlaceholder(props);

    const documentClick = (e: MouseEvent) => {
      e.stopPropagation();

      if (isIn(e.target as Node, panel.value) || isIn(e.target as Node, input.value)) {
        return;
      }
      state.show = false;
    };
    ...
    return () => {
      return (
        <div class="nancalui-datepicker-container">
          ...
          <div class="nancalui-datepicker-panel" ref={panel}>
            {state.show ? (
              <Calendar
                ...
                onChange={({ value }) => {
                  ...
                  state.value = formatValue(value, state, props);
                  state.placeholder = formatPlaceholder(props);
                ...
                onToday={(date: Date) => {
                  state.current = date;
                  state.start = date;
                  state.value = formatValue(date, state, props);
                ...
                onPreviousYear={(date: Date, pos: number) => {
                  handleCalendarSwitchState(state, 0, pos, date);
                }}
