...
const VerticalSlider = defineComponent({
  ...
  setup(props: VerticalSliderProps) {
    ...
    return () => {
      return (
        <div ref={container} class={`nancalui-vertical-slider ${className}`}>
          <div
            ref={movbar}
            class="movable-bar"
            style={{
            ...
            {items.map((c, i) => {
              ...
              return (
                <span class={`slider-item ${curClassName}`} style={{ height: `${size}px`, lineHeight: `${size}px` }}>
                  {c}
                </span>
              );
            })}
