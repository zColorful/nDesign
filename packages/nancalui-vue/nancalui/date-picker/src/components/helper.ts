  ...
  if (props.type === 'range') {
    if (!props.dateStart) {
      ...
      if (subDateDay(props.dateStart, day.date) !== 0) {
        ...
        typeof props.onChange === 'function' && props.onChange({ type: props.type, value: day.date });
      }
    } else {
    ...
    typeof props.onChange === 'function' && props.onChange({ type: props.type as TDatePanelType, value: day.date });
  }
};
