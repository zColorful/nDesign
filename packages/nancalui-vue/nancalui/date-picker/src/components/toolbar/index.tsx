...
const Item = (props: TCalendarToolbarItemProps) => {
  const { button: Btn, disabled = false, rotate = 0, date, pos } = props;
...
export const Title = (props: { date: Date }): JSX.Element => {
  const { date } = props;
  return <a class="title">{`${date.getFullYear()}年${(date.getMonth() + 1 + '').padStart(2, '0')}月`}</a>;
};

const CalendarToolbar = (props: TDateToolbarProps): JSX.Element => {
  const { type, current, compare, pos, dateMax, dateMin, onPreviousYear, onPreviousMonth, onNextMonth, onNextYear } = props;
  ...
  return (
    <div class="nancalui-calendar-toolbar">
