...
const StickSlider = defineComponent({
  ...
  setup() {
    ...
    return () => {
      return (
        <div
          class="nancalui-stick-slider"
          onMousedown={handleMainButtonMouseDown}
          onMouseup={handleMainButtonMouseUp}
          onMouseleave={handleMainButtonMouseUp}>
          <div class="sub-buttons" style={{ display: state.showButtons ? '' : 'none' }}>
            {Array(16)
              .fill(null)
              .map((_, i) => {
                return (
                  <div class={`button ${i === state.selectedIndex ? 'selected' : ''}`} onMouseenter={() => (state.selectedIndex = i)}>
                    {i}
                  </div>
                );
              })}
