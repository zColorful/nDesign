...
import { IInnerTreeNode, IUseCore, IUseSearchFilter, SearchFilterOption, ITreeNode } from './use-tree-types';
import { trim } from 'lodash';
export function useSearchFilter(prop: { label: keyof ITreeNode }) {
  ...
  return function useSearchFilterFn(data: Ref<IInnerTreeNode[]>, core: IUseCore): IUseSearchFilter {
    ...
    const dealMatchedData = (target: string, matchKey: string | undefined, pattern: RegExp | undefined) => {
      ...
      for (let i = 0; i < data.value.length; i++) {
        const key = matchKey ? data.value[i][matchKey] : data.value[i][prop.label];
        ...
        if (selfMatched) {
          data.value[i].matchedText = matchKey ? data.value[i][prop.label] : trimmedTarget;
