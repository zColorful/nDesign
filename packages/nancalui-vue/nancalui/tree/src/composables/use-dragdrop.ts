...
export function useDragdrop(props: TreeProps, data: Ref<IInnerTreeNode[]>) {
  return function useDragdropFn(): IUseDraggable {
    ...
    const handlerDropData = (dragNodeId: string | number, dropNodeId: string | number, currentDropType?: string) => {
      ...
      const findDragAndDropNode = (curr: ITreeNode[], parentDropNode: ITreeNode) => {
        ...
        curr.every((item, index) => {
          ...
          if (item.id === dragNodeId) {
            ...
            currentDropNode = { target: curr, index, item, parentNode: parentDropNode }; // 获取落入节点的兄弟姐妹节点数组，落入节点的index和落入节点的数据
          }
          if (!currentDragNode || !currentDropNode) {
            Array.isArray(item.children) && findDragAndDropNode(item.children, item);
          }
          return true;
        });
      };
      ...
      if (currentDragNode && currentDropNode && currentDropType) {
        ...
        if (currentDropType === 'dropPrev') {
          ...
          if (currentDropNode.parentNode) {
            currentDropNode.parentNode.expanded = true;
          }
          ...
          currentDropNode.target[currentDropNode.index].expanded = true;
    ...
    const onDrop = (event: DragEvent, dropNode: IInnerTreeNode, isUse = true): void => {
      event.preventDefault();
      event.stopPropagation();
      removeDraggingStyle(event.currentTarget as HTMLElement | null);
      if (isUse) {
