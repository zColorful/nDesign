...
export function useCore(): (data: Ref<IInnerTreeNode[]>, initData: any, childrenName: string) => IUseCore {
  const nodeMap = new Map<string, IInnerTreeNode[]>();

  return function useCoreFn(data: Ref<IInnerTreeNode[]>, initData, childrenName: string): IUseCore {
    ...
    const getNode = (node: IInnerTreeNode, key: keyof IInnerTreeNode): IInnerTreeNode => {
      key = key ? key : 'id';
      return data.value.find((item) => item[key] === node[key]);
    };
    const saveExpanded = (vdata: ITreeNode[], node: IInnerTreeNode, value: any) => {
      for (let i = 0; i < vdata.length; i++) {
        if (vdata[i].id === node.id) {
          vdata[i].expanded = value;
          break;
        } else if (vdata[i]?.[childrenName]?.length) {
          saveExpanded(vdata[i][childrenName] || [], node, value);
        }
      }
      return vdata;
    };
    ...
    const setTree = (newTree: ITreeNode[]): void => {
      clearNodeMap();
      data.value = generateInnerTree(newTree, childrenName);
    };
