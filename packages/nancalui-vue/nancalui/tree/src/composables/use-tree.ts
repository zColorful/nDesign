...
export function useTree(tree: ITreeNode[], plugins = [], context: SetupContext, data: unknown, childrenName: string): Partial<IUseTree> {
  const treeData = ref<IInnerTreeNode[]>(generateInnerTree(tree, childrenName));
  const core: IUseCore = useCore()(treeData, data,childrenName);
