...
export function generateInnerTree(tree: ITreeNode[], key = 'children', level = 0, path: ITreeNode[] = [], groupName): IInnerTreeNode[] {
  level++;

  const result = tree.reduce((acc: IInnerTreeNode[], item: ITreeNode, currentIndex) => {
    ...
    newItem.groupName = level === 1 ? newItem.id : groupName;
    newItem.level = level;
    newItem.parentChildNodeCount = tree.length;
    newItem.currentIndex = currentIndex;
    newItem.childNodeCount = newItem[key]?.length || 0;
    ...
    if (!newItem[key]) {
      ...
      return acc.concat(omit<ITreeNode>(newItem, key), generateInnerTree(newItem[key], key, level, path, newItem.groupName));
    }
  }, []);
