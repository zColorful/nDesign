...
import NTagInput from '../src/tag-input';
...
const customMount = (state: StateType) =>
  mount({
    components: { NTagInput },
    template: `
    <n-tag-input
      v-model="state.tags"
...
describe('NTagInput', () => {
  it('tag-input render work', async () => {
    const state = reactive({
      tags: [{ cname: 'Y.Chen' }, { cname: 'b' }, { cname: 'c' }],
      suggestionList: [{ cname: 'd' }, { cname: 'e' }, { cname: 'f' }],
    });
    const wrapper = customMount(state);
    expect(wrapper.find(ns.b()).exists()).toBe(true);
    expect(wrapper.find('.nancalui-tags').exists()).toBe(true);
    expect(wrapper.find('.nancalui-tag-list').exists()).toBe(true);
    expect(wrapper.find('.nancalui-input').exists()).toBe(true);

    const itemA = wrapper.find('.nancalui-tag-item');
  ...
  it('tag-input show suggestion work', async () => {
    const state = reactive({
      tags: [{ cname: 'a' }],
      suggestionList: [{ cname: 'b' }],
    });
    const wrapper = customMount(state);
    const input = wrapper.find('input.nancalui-input');

    expect(wrapper.find('.nancalui-suggestion-list').exists()).toBe(false);
    await input.trigger('focus');

    // 是否存在 nancalui-suggestion-list
    const suggestionList = !!document.querySelectorAll('.nancalui-suggestion-list')[0];
  ...
  it('tag-input disabled work', async () => {
    const tags = reactive([{ cname: 'a' }]);
    const suggestionList = reactive([{ cname: 'b' }]);
    const wrapper = mount(NTagInput, {
      props: {
        tags,
        suggestionList,
        disabled: false,
    ...
    expect(wrapper.find('.nancalui-disabled').exists()).toBe(false);
    expect(wrapper.find('.nancalui-input').isVisible()).toBe(true);

    await wrapper.setProps({
      disabled: true,
    });
    expect(wrapper.find('.nancalui-disabled').exists()).toBe(true);
    expect(wrapper.find('.nancalui-input').isVisible()).toBe(false);
  ...
  it('tag-input maxTags work', () => {
    const tags = reactive([{ cname: 'a' }, { cname: 'b' }]);
    const suggestionList = reactive([{ cname: 'c' }]);
    const wrapper = mount(NTagInput, {
      props: {
        modelValue: tags,
        suggestionList,
  ...
  it('tag-input removeTag work', async () => {
    const state = reactive({
      tags: [{ cname: 'a' }, { cname: 'b' }],
      suggestionList: [{ cname: 'c' }],
    });
    const wrapper = customMount(state);
    const removeSvg = wrapper.find('.remove-button');
    await removeSvg.trigger('click');
    expect(wrapper.findAll('.nancalui-tag-item').length).toBe(1);
  ...
  it('tag-input keydown work', async () => {
    const state = reactive({
      tags: [{ cname: 'a' }, { cname: 'b' }],
      suggestionList: [{ cname: 'c' }, { cname: 'xyz' }],
    });
    const wrapper = customMount(state);
  ...
  it('tag-input filter suggestion work', async () => {
    const state = reactive({
      tags: [{ cname: 'a' }, { cname: 'b' }],
      suggestionList: [{ cname: 'x' }, { cname: 'xy' }, { cname: 'xyz' }],
    });
    const wrapper = customMount(state);
    const input = wrapper.find('input');

    await input.trigger('focus');
    let suggestionList = document.querySelectorAll('.nancalui-suggestion-item');
    expect(suggestionList.length).toBe(3);

    await input.setValue('xy');
    await input.trigger('input');
    suggestionList = document.querySelectorAll('.nancalui-suggestion-item');
  ...
  it('tag-input click suggestion work', async () => {
    const state = reactive({
      tags: [{ cname: 'a' }, { cname: 'b' }],
      suggestionList: [{ cname: 'x' }, { cname: 'yyy' }, { cname: 'xyz' }],
    });
    const wrapper = customMount(state);
    await wrapper.find('input').trigger('focus');
    const suggestionList = document.querySelectorAll('.nancalui-suggestion-item');
