...
export default defineComponent({
  ...
  setup(props: TagInputProps, ctx: SetupContext) {
    ...
    const chosenTags = () => {
      return (
        <ul class="nancalui-tag-list" title={props.disabled ? props.disabledText : ''}>
          {selectedTags.value.map((tag, tagIdx) => {
            return (
              <li class="nancalui-tag-item">
    ...
    const suggestionList = () => {
      ...
      const suggestionListItem = mergedSuggestions.value.map((item: Suggestion, index: number) => {
        return (
          <li
            class={{ 'nancalui-suggestion-item': true, selected: index === selectIndex.value }}
            onClick={($event: Event) => {
      ...
      const noDataTpl = <li class="nancalui-suggestion-item nancalui-disabled">{props.noData}</li>;
        ...
        <Teleport to="body">
          <Transition name="fade">
            <FlexibleOverlay
              ...
              <div class="nancalui-tags-autocomplete" style={{ width: `${dropdownWidth.value}` }}>
                <ul class="nancalui-suggestion-list">{showNoData ? noDataTpl : suggestionListItem}</ul>
              </div>
            </FlexibleOverlay>
          </Transition>
        </Teleport>
    ...
    const inputBoxCls = computed(() => {
      return {
        'nancalui-tags': true,
        'nancalui-form-control': true,
        'nancalui-dropdown-origin': true,
        'nancalui-dropdown-origin-open': isInputBoxFocus.value,
        'nancalui-disabled': props.disabled,
      };
    });
