...
export function useTextareaRender(props: TextareaProps): UseTextareaRender {
  ...
  const { error, disabled, noborder } = toRefs(props);
  const textareaDisabled = computed(() => disabled.value || formContext?.disabled);
  const textareaDisabledBorder = computed(() => noborder.value || formContext?.noborder);

  const wrapClasses = computed(() => ({
    [ns.b()]: true,
    [ns.m('focus')]: isFocus.value,
    [ns.m('disabled')]: textareaDisabled.value,
    [ns.m('noborder')]: textareaDisabledBorder.value,
  ...
  return { isFocus, textareaDisabled, textareaDisabledBorder, wrapClasses };
}
