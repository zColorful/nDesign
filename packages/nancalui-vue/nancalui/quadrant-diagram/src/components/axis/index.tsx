...
export default defineComponent({
  ...
  setup(props: QuadrantDiagramAxisProps) {
     ...
     */
    const initAxisData = () => {
      quadrantAxis.value = document.querySelector('#nancalui-quadrant-axis-' + diagramId?.value);
    };
      ...
      for (let i = 0; i <= axisInnerAttr.xAxisTicksNum; i++) {
        if (i % (axisConfigsVal.xAxisRange?.step || 0) === 0) {
          context.value.fillText(
            i,
            axisInnerAttr.axisOrigin.x + i * axisInnerAttr.xTickSpacing,
    ...
    const drawYTicksLabels = () => {
      ...
      for (let i = 0; i <= axisInnerAttr.yAxisTicksNum; i++) {
        if (i % (axisConfigsVal.xAxisRange?.step || 0) === 0) {
          context.value.fillText(
            i,
            axisInnerAttr.axisOrigin.x - (axisConfigsVal.spaceBetweenLabelsAxis || 0),
    ...
    const drawAxisTitle = () => {
      ...
      rotateLabel(
        axisConfigsVal.xAxisLabel || '',
        axisInnerAttr.axisRight + (axisConfigsVal.axisMargin || 0) / 2,
        axisInnerAttr.axisOrigin.y - xLabelWidth - AXIS_TITLE_SPACE
      );
      context.value.fillText(
        axisConfigsVal.yAxisLabel,
        axisInnerAttr.axisOrigin.x + AXIS_TITLE_SPACE,
        axisInnerAttr.axisTop - (axisConfigsVal.axisMargin || 0) / 2
      );
    };
  ...
  render() {
    ...
    return (
      <div>
        <canvas id={'nancalui-quadrant-axis-' + diagramId} height={view?.height} width={view?.width}></canvas>
      </div>
    );
  },
});
