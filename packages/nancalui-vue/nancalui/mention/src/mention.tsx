...
import NTextarea from '../../textarea/src/textarea';
...
export default defineComponent({
  ...
  setup(props: MentionProps, { slots, emit }) {
    ...
    const handleBlur = (e: Event) => {
      const { target } = e;
      const ele = document.querySelector('.nancalui-mention');
    ...
    const arrowKeyDown = (e: KeyboardEvent) => {
      if (showSuggestions.value && filteredSuggestions.value.length) {
        ...
        const itemDom = instance?.proxy?.$refs[`nancalui-suggestions-item-${currentIndex.value}`];
    ...
    return () => {
      return (
        <div class={ns.b()}>
          <n-textarea v-model={textContext.value} onUpdate={handleUpdate} onFocus={handleFocus}></n-textarea>
              ...
              <div
                ...
                {filteredSuggestions.value.length > 0 ? (
                  filteredSuggestions.value?.map((item, index) => {
                    return (
                      <div
                        ref={`nancalui-suggestions-item-${index}`}
