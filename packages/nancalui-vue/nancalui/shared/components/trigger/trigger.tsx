...
import { usePickSlots } from '../../_utils/use-pick-slots';
import { triggerInjectionKey } from './context';
import { throttleByRaf } from '../../_utils/throttle-by-raf';
import { usePopupManager } from '../../_utils/use-popup-manager';
import { useResizeObserver } from '../../_utils/use-resize-observer';
import { useTeleportContainer } from '../../_utils/use-teleport-container';
import { TriggerEvent, TriggerPopupTranslate, TriggerPosition } from './interface';
import { useFirstElement } from '../../_utils/use-first-element';
...
export default defineComponent({
  ...
  setup(props, { emit, slots, attrs }) {
    ...
    const updatePopupStyle = () => {
      ...
      const getPopupRect = () => getElementScrollRect(popupRef.value as any, containerRect);
    ...
    const addChildRef = (child: Ref<HTMLElement>) => {
      childrenRefs.add(child);
      triggerCtx?.addChildRef(child);
    };
    const removeChildRef = (child: Ref<HTMLElement>) => {
      childrenRefs.delete(child);
      triggerCtx?.removeChildRef(child);
    };
                ...
                <ResizeObserver onResize={handleResize}>
                  <div
                    ...
                    style={{
                      ...popupStyle.value,
                      zIndex: zIndex.value,
                      pointerEvents: isAnimation.value || !props.popupHoverStay ? 'none' : 'auto',
                    }}
                    ...
                    <Transition
                      ...
                      <div class={`${prefixCls}-popup-wrapper`} style={transformStyle.value} v-show={computedVisible.value}>
                        ...
                        {props.showArrow && (
                          <div ref={arrowRef} class={[`${prefixCls}-arrow`, props.arrowClass]} style={arrowStyle.value} />
                        )}
                      </div>
                    </Transition>
                  </div>
