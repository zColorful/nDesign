...
import NAutoComplete from '../src/auto-complete';
...
describe('auto-complete', () => {
  it('init render & KeyboardEvent ', async () => {
    const wrapper = mount({
      components: { 'n-auto-complete': NAutoComplete },
      template: `
        <n-auto-complete
          :source="source"
          v-model="value"
  ...
  it('Customized data matching method ', async () => {
      ...
      template: `
        ...
        >
          <template #searching="slotProps" >
            <div id="nancalui-is-searching-template">
                {{slotProps}}
            </div>
          </template>
        </n-auto-complete>
    ...
    await nextTick();
    await wait(300);
    expect(wrapper.find('#nancalui-is-searching-template').exists()).toBe(true);
    expect(wrapper.find('#nancalui-is-searching-template').element.innerHTML).toBe('c');
    await wait(500);
    await nextTick();
  ...
  it('n-auto-complete prefix/suffix props work', async () => {
    const wrapper = mount({
      components: { NAutoComplete },
      template: `
        <n-auto-complete prefix="like" suffix="search" />
      `,
    });
  ...
  it('n-auto-complete prefix/suffix/prepend/append slot work', async () => {
    const wrapper = mount({
      components: { NAutoComplete },
      template: `
        <n-auto-complete>
          <template #prepend>
            <n-button icon="like">测试</n-button>
          </template>
          <template #prefix>
            <n-icon name="search" />
          </template>
  ...
  it('n-auto-complete clearable/clear work', async () => {
    const onClear = jest.fn();
    const wrapper = mount({
      components: { NAutoComplete },
      template: `
        <n-auto-complete @clear="onClear" clearable/>
      `,
      setup() {
        return {
          onClear,
  ...
  it('n-auto-complete size work', async () => {
  ...
  it('n-auto-complete bindEvents work', async () => {
