...
export function useOverlay(props: FlexibleOverlayProps, emit: EmitEventFn): UseOverlayFn {
  ...
  const updatePosition = async () => {
    ...
    const offsetWidth = placement === 'bottom-end' ? 0.5 * overlayEl.offsetWidth : 0;
    let applyX = x - offsetWidth;
    let applyY = y;

    if (hostEl?.className && typeof hostEl.className === 'string') {
      if (hostEl.className.includes('nancalui-tree-select')) {
        const windowHeight = window.innerHeight;
        if (applyY > windowHeight / 2) {
          applyY = applyY - overlayEl.scrollHeight;
        }
      }
    }
