...
export default defineComponent({
  ...
  setup(props: TimelineProps, ctx) {
    ...
    const setStyle = () => {
      ...
      if (props.direction === 'horizontal') {
        nextTick(() => {
          ...
          style.height =
            Math.max(
              ...Array.from(element?.querySelectorAll('.nancalui-timeline-item-data-top')).map((el) => el.clientHeight),
              ...Array.from(element?.querySelectorAll('.nancalui-timeline-item-data-bottom')).map((el) => el.clientHeight)
            ) *
              2 +
            Math.max(...Array.from(element?.querySelectorAll('.nancalui-timeline-item-axis')).map((el) => el.clientHeight)) +
            'px';
        });
      }
    };
    ...
    return () => {
      ...
      return (
        <div
          class={`nancalui-timeline nancalui-timeline-${getDirection()}  ${
            props.center ? 'nancalui-timeline-' + getDirection() + '-center' : ''
          } `}
          ref={timeAxis}
          style={style}>
          {renderItem()}
